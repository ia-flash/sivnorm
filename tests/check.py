...
from sivnorm.process import marques_dict

def test_check():

    df_in = pd.read_csv('./check.csv',names=['marque','modele'],usecols=[0,1], encoding='utf-8')
    df_ref = pd.read_csv('./check.csv',names=['marque','modele'],usecols=[2,3], encoding='utf-8')
    df_in = df_in.fillna("")
